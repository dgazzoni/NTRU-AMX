...
if [ -d ct_results_${CPU}_no_dit ]
then
    # https://stackoverflow.com/a/226724/523079
    echo "Existing results will be deleted. Are you sure?"
    select yn in "Yes" "No"; do
        case $yn in
            Yes ) rm -rf ct_results_${CPU}_no_dit; break;;
            No ) exit;;
        esac
    done
fi

mkdir ct_results_${CPU}_no_dit
    ...
    do
        ...
        do
            ...
            do
                ...
                # Actual run
                ./speed > \
                    "../ct_results_${CPU}_no_dit/ntru:${PARAMETER_SET}:${ALLOC}:${IMPL}:${VARIANT}.txt"
...
chown -R $(logname) build/ ct_results_${CPU}_no_dit/
